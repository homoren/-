...
<div class="page-content">
    <form action="<%=path%>/noticeinfo_list.action" class="form-inline" method="post">
        <div class="panel panel-default">
            <div class="panel-heading">网站公告列表</div>
            ...
            <pg:pager url="noticeinfo_list.action" items="${itemSize}" maxPageItems="${pageItem}"
                        ...
                        <c:forEach items="${list}" var="info">
                            <tr>
                                <td><a style="color:#000000"
                                       href="<%=path%>/noticeinfo_toView.action?id=${info.id}">${info.title}</a></td>
                                <td>${info.updatetime}</td>
                                <td>
                                    <a href="<%=path%>/noticeinfo_toUpdate.action?id=${info.id}"
...
<script type="text/javascript">
    $(document).ready(function () {
        $("#addBtn").click(function () {
            window.location.href = '<%=path%>/noticeinfo_toAdd.action';
        });
    });

    function delInfo(id) {
        if (confirm("是否确认删除？")) {
            window.location.href = '<%=path%>/noticeinfo_delete.action?id=' + id;
        }
    }
</script>